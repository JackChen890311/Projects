...
req <- GET('https://www.dcard.tw/',
           path = c("_api","forums","makeup","posts?popular=false&limit=100"))
idx <- content(req)
...
for(loop in 2:10)
{
  str <- paste0("https://www.dcard.tw/_api/forums/makeup/posts?popular=false&limit=100&before=", id_end)
  req <- GET(str)
  idx <- content(req) 
  for(count in 1:100)
  {
...
# 記下所爬到Dcard最早文章的時間，方便之後PTT做對照
T_end <- content(GET('https://www.dcard.tw/', path = c("_api",
                                                 "posts",idxs[1000])))$createdAt
T_end

#由爬PTT的結果（另外進行）得知，在與Dcard同樣時段內僅有211篇文，故取樣211篇

S_dcard <- sample(1:1000,size = 211)
idxs<-idxs[S_dcard]


# 藉由idxs中存取的211篇文章之id，將那些文章的「發文時間」、「文章內容」分別存入相對應的vector
time <- vector(mode = "character",length = 211)
dcard_content <- vector(mode = "character",length = 211)
for(i in 1:211)
{
...
title <- paste0("cosine similarity(Dcard,makeup)\nmedian:",median(dcard_cosine_similarity))
...
# PTT  美妝版爬到之前Dcard所爬到最早的時間（確保資料同為相同時段）
...
# 對所有的Cosine Similarity作圖
title <- paste0("cosine similarity(PTT,MakeUp)\nmedian:",median(ptt_cosine_similarity))
