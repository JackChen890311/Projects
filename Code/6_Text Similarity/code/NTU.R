...
req <- GET('https://www.dcard.tw/',
           path = c("_api","forums","ntu","posts?popular=false&limit=100"))
idx <- content(req)
...
for(loop in 2:10)
{
  str <- paste0("https://www.dcard.tw/_api/forums/ntu/posts?popular=false&limit=100&before=", id_end)
  req <- GET(str)
  idx <- content(req) 
  for(count in 1:100)
  {
...
title <- paste0("cosine similarity(Dcard,NTU)\nmedian:",median(dcard_cosine_similarity))
...
# PTT NTU 版爬到之前Dcard所爬到最早的時間（確保資料同為相同時段）
...
# 對所有的Cosine Similarity作圖
title <- paste0("cosine similarity(PTT,NTU)\nmedian:",median(ptt_cosine_similarity))
